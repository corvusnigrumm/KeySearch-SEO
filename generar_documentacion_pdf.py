...
bullet_style = ParagraphStyle("BulletCustom", parent=body_style, leftIndent=15, spaceAfter=3)
...
story.append(
    Paragraph(
        "&bull; <b>KeywordPlanIdeaService (GenerateKeywordHistoricalMetrics):</b> Retrieves real historical monthly search volumes, competition level, competition index, and low/high Top of Page bid estimates for the article-related keywords.",
        bullet_style,
    )
)
story.append(
    Paragraph(
        "&bull; <b>GeoTargetConstantService (SuggestGeoTargetConstants):</b> Resolves country codes (e.g. 'CO') into Google Ads geo target resource names for geographically targeted campaigns.",
        bullet_style,
    )
)
story.append(
    Paragraph(
        "&bull; <b>GoogleAdsService (language_constant query):</b> Retrieves language resource names for precise language targeting in ad campaigns.",
        bullet_style,
    )
)
...
story.append(
    Paragraph(
        "Below is a structural representation of the campaign output generated by KeySearch for each published article:",
        body_style,
    )
)
...
header_data = [
    [
        Paragraph(
            "<b><font color='#FFFFFF' size=11>KeySearch - Google Ads Campaign Export (Ads Editor Format)</font></b>",
            body_style,
        )
    ]
]
